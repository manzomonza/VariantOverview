...
read_input_tsv = function(prep_snvs){
  ...
  if(length(tsvs) == 1){
    ...
    dplyr::select(uid, homopolymer_length,ref_ref_var_var,phred_qual_score,ucsc_common_sn_ps)
      toi$name = base_name
...
variants_parameters = dplyr::left_join(count_variants, compl) %>%
  dplyr::select(chr, start, end, gene,coding, amino_acid_change, locus,homopolymer_length:coverage, pcr_id.x, Bno.x)
